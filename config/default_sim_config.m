...
%% 8. 分析
config.analysis.enablePointAnalysis = false;   % [bool] 是否执行点目标分析
config.analysis.enableImageQuality = false;    % [bool] 是否执行图像质量评估
...
config.analysis.recoveryEvaluation = struct( ...
    'enable', false, ...                       % [bool] 是否启用恢复效果评估
    'evaluationMode', 'simulation', ...        % 'real' | 'simulation'
    'caseNames', {{'full', 'interrupted', 'recovered_cs_1d', 'recovered_cs_2d'}}, ...
    'referenceCase', 'full', ...               % 对比参考 case
    'outputs', struct( ...
        'saveMat', true, ...                   % [bool] 是否保存结构化评估结果
        'saveSummary', true, ...               % [bool] 是否保存简短 summary 文本
        'savePanel', true, ...                 % [bool] 是否保存总览对比图
        'showPanel', false), ...               % [bool] 是否直接显示总览对比图
    'metricOptions', struct( ...
        'enablePointAnalysis', false, ...      % [bool] 评估内部是否强制做点目标分析
