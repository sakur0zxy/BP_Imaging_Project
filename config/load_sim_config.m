function config = load_sim_config(userOverrides)
%LOAD_SIM_CONFIG 加载仿真配置。
...
config = normalize_legacy_config_aliases(config, 'sim'); % 兼容旧项目字段名
config = validate_sim_config(config); % 最后统一校验
end
