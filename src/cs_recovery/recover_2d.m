function result = recover_2d(model, config)
%RECOVER_2D 使用二维 FFT 稀疏先验恢复缺失回波。
localValidateInputs(model, config);

lambda = config.lambda2D;
[echoRec, info] = localRunFista(model.observedEcho, model.observedMatrixMask, lambda, ...
    @localProx2D, @localTransform2D, config, '2d-fft');
...
assert(isfield(config, 'lambda2D') && config.lambda2D >= 0, ...
    'config.lambda2D 必须大于等于 0。');
end
